...
if( isset($_REQUEST['submit']) ){
   if ($_POST['submit'] == 'delete'){
       $sql 	= "DELETE FROM  ". $wpdb->prefix ."ratetingstar WHERE id = ".$_POST['id'];
...
<!--            <a class="page-title-action" href="--><?php //echo $module_path; ?><!--&sub=add">Thêm mới</a>-->
        ...
        <ul class="subsubsub">
            <li class="all"><a class="current" href="<?php echo $module_path; ?>">Tất cả <span class="count">(<?php echo $recordcount;?>)</span></a></li>
        </ul>

<!--        <form class="search-box flr" style="text-align: right" method="POST" action="--><?php //echo $module_path; ?><!--">-->
<!--            <input class="sear_2" value="--><?php //if(isset($keyword)) echo $keyword ;?><!--" type="text" name="keyword" placeholder="Từ khóa">-->
<!---->
...
<!--        </form>-->
        ...
        <?php
        $sql 	= "SELECT * FROM  ". $wpdb->prefix ."ratetingstar ".$my_str." ORDER BY id DESC limit ".$beginpaging[0].",$pagesize";
        ...
        <form class="" method="POST" action="<?php echo $module_path; ?>">
            ...
            <table class="wp-list-table widefat fixed striped posts">
                <thead>
                <tr class="headline">
                    <td class="manage-column column-cb check-column" id="cb"><input type="checkbox" id="cb-select-all-1"></td>
                    <th style="width:30px;text-align:center;">STT</th>
                    <th style="width: 15%">Họ tên</th>
                    <th style="width: 20%">Tiêu đề</th>
                    <th style="width: 35%">Nội dung</th>
                    <th style="width: 5%">Số sao</th>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <form action="" method="post">
                                ...
                                if ($row->status == 0){
                                    ?>
                                    <button type="submit" class="buttton_duyet" name="submit" onclick="return confirm('Bạn có chắc muốn thực hiện hành động này?');" value="duyet" >Duyệt</button>
                                    <?php
                                }
                                ?>
                                <button type="submit" name="submit" value="delete" onclick="return confirm('Bạn có chắc muốn thực hiện hành động này?');"  class="buttton_xoa" >Xóa bỏ</button>
                            </form>
