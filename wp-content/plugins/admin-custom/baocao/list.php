...
<div class="wrap">
    <h1 style="margin-bottom:15px;">Danh sách <?php echo $mdlconf['title']; ?> đã nộp
<!--        <a class="page-title-action" href="--><?php //echo $module_pathadd; ?><!--">Thêm mới</a>-->
    ...
    <form class="search-box flr" method="POST" action="<?php echo $module_path; ?>">
        ...
        <input type="button" name="exportTotal" id="exportTotal" value="Xuất báo cáo" class="button mr-5"/>
        <input style="display: none" type="button" name="excel" value="Xuất báo cáo" id="xuat_excel" class="button mr-5"/>
...
//                $user = $wpdb->get_row("select * from  ".$wpdb->prefix."account where  id = '".$item->id_user."'")
                ...
                <tr>
                    ...
                    <td>
                        <a target="_blank" href="<?= get_permalink(getIdPage('chitiet-baocao')) ?>?id=<?= $item->id ?>">Xem</a>
                    </td>
                    <td><a href="javascript:void(0)" class="remove-report" data-id="<?= $item->id ?>">Xóa</a></td>
                    <td><a href="javascript:void(0)" class="file-report" data-json="<?= $data_json->report7->img ?>" data-id="<?= $item->id ?>">Xuất file đính kèm báo cáo</a></td>
                </tr>
            <?php } ?>
        </table>
                                ...
                                <div class="submit-bt">
                                    ...
                                    <div class="report-pt">
                                        ...
                                        <div class="btn-button">
                                            <button class="button-item" data-id="1">Xuất báo cáo</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="report-pt">
                                        ...
                                        <?php endforeach; ?>
                                        <div class="btn-button">
                                            <button class="button-item" data-id="2">Xuất báo cáo</button>
                                        </div>
                                    </div>
                                    ...
                                    <div class="report-pt">
                                        ...
                                        <div class="btn-button">
                                            <button class="button-item" data-id="3">Xuất báo cáo</button>
                                        </div>
                                    </div>
...
<div class="modal fade" id="myModal-4" role="dialog">
    <div class="modal-dialog" style="">
        ...
        <div class="modal-content" style="overflow: auto; height: 600px">
            <div class="modal-header">
                <button style="margin-top: -10px;" type="button" data-close="myModal-4" class="close close-notdata" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="grid simple">
                            <div class="grid-title">
                                <div class="submit-bt">
                                    ...
                                    <div class="btn-button">
                                        <button class="downloadZipFile">Tải xuống</button>
                                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default close-notdata" data-close="myModal-4" data-dismiss="modal">Thoát</button>
            </div>
        </div>
...
<style>
    ...
    .btn-button .button-item, .btn-button .downloadZipFile{
        border: unset;
        background: #019fe2;
        border-radius: 6px;
        color: #fff;
...
<script>
    ...
    $("body").on("click", ".file-report", function () {
        ...
        arrayFileDK = [];
        grecaptcha.ready(function() {
            grecaptcha.execute(setting_captcha, {action: 'subscribe_fileReport'}).then(function(token) {
                $.ajax({
                    url: urlAjax,
                    type: 'POST',
                    cache: false,
                    ...
                    success: function (rs) {
                        ...
                        if (rs.status == '0') {
                            ...
                            var html = "";
                            rs.report.map(function (value, index) {
                                var metaValueJson = JSON.parse(value.meta_value.replace("\t",""));
                                // var arrtxt = value.meta_key.split("");
                                html += "<div><p>Mục "+value.txt+"</p>";
                                var html2 = "";
                                metaValueJson.fieldType3.map(function (value1, index1) {
                                    var text = value1.split("/");
                                    html2 += '<div class="br-checkbox">\n' +
                                        ' - <label><a href="'+value1+'">'+text[text.length - 1]+'</a></label>\n' +
                                        ' </div>'
                                    arrayFileDK.push(value1);
                                });
                                html += html2 + "</div>";
                            });
                            var text2 = json.split("/");
                            html += '<div><p>Mục 7: ĐÁNH GIÁ KẾT QUẢ THỰC HIỆN, NHỮNG KHÓ KHĂN, TỒN TẠI VÀ KIẾN NGHỊ</p><div class="br-checkbox">\n' +
                            ' - <label><a target="_bank" href="'+json+'">'+text2[text2.length - 1]+'</a></label>\n' +
                            ' </div></div>';
                            arrayFileDK.push(json);
                            $(".report-pt").html(html);
                            //Modal
                            $("#myModal-4").addClass("in");
                            $("#myModal-4").slideDown(500);
                            $(".overlay").addClass("overlay-active");
                            //End Modal
                        }else {
                            Swal.fire({
    ...
    $("body").on("click",".downloadZipFile", function () {
        $('.divgif').css('display', 'block');
        var zipFilename = "File-đinh-kem";
        var zip = new JSZip();
        var count = 0;
        let link = arrayFileDK;
